...
test('已有数据库迁移到新位置：数据完整、路径更新、旧文件清理', async () => {
  ...
  assert.equal(res.newPath, path.join(path.resolve(newDir), 'comate.db'))

  // 路径配置已更新
  assert.equal(dbPath(), path.join(path.resolve(newDir), 'comate.db'))
  // 数据完整
  const db2 = getDb()
  ...
  // 旧文件已清理、新文件存在
  assert.ok(!fs.existsSync(path.join(oldDir, 'comate.db')), '旧 comate.db 应被清理')
  assert.ok(fs.existsSync(res.newPath), '新位置 comate.db 应存在')
})
...
test('目标已存在数据库文件：未强制返回 need_confirm，force 后覆盖迁移', async () => {
  ...
  fs.writeFileSync(path.join(newDirB, 'comate.db'), 'placeholder')

  // 未强制 → need_confirm
  const info = await moveDb(newDirB)
  assert.equal(info.status, 'need_confirm')
  assert.ok(info.message && info.message.length > 0)
  assert.equal(dbPath(), path.join(path.resolve(newDirA), 'comate.db'), '确认前路径不应改变')

  // force → 覆盖迁移成功
  const res = await moveDb(newDirB, { force: true })
  assert.equal(res.status, 'ok')
  assert.equal(res.action, 'migrate')
  assert.equal(dbPath(), path.join(path.resolve(newDirB), 'comate.db'))
...
test('当前无数据库：在新位置自动初始化', async () => {
  ...
  getDb().close()
  fs.rmSync(path.join(oldDir, 'comate.db'), { force: true })
  assert.ok(!fs.existsSync(path.join(oldDir, 'comate.db')))
  ...
  assert.equal(dbPath(), path.join(path.resolve(newDir), 'comate.db'))
  assert.ok(fs.existsSync(path.join(path.resolve(newDir), 'comate.db')), '新位置应创建数据库')
  // 新库已完成初始化（种子分类存在）
